...
class PorgramDetailApprovalController extends Controller
{
    public function index()
    {
        $userId = Auth::id();
        // dd($userId);
        $approvedDocs = ApprovalWorkflows::getQueriedResult('App\Models\ProgramDetail', ['phc_approve', 'block_approve', 'hud_approve', 'state_approve']);

        $results = ApprovalWorkflows::getQueriedResult('App\Models\ProgramDetail');
        $pushedDocs = ApprovalWorkflows::getQueriedResult('App\Models\ProgramDetail', ['published']);
        $verifiedDocs = ApprovalWorkflows::getQueriedResult('App\Models\ProgramDetail', ['phc_verify', 'block_verify', 'hud_verify', 'state_verify']);
        $pendingDocs = ApprovalWorkflows::getQueriedResult('App\Models\ProgramDetail', ['hsc_upload', 'phc_upload', 'block_upload', 'hud_upload', 'state_upload']);
        $returnedDocs = ApprovalWorkflows::getQueriedResult('App\Models\ProgramDetail', ['returned_with_remarks']);
        $rejectedDocs = ApprovalWorkflows::getQueriedResult('App\Models\ProgramDetail', ['rejected_with_remarks']);
        // dd($approvedDocs['paginatedResult']->toarray());
        return view('admin.approval-management.programdetail.list', compact('results', 'pushedDocs', 'verifiedDocs', 'approvedDocs', 'pendingDocs', 'returnedDocs', 'rejectedDocs', 'userId'));
    }
    ...
    public function handleAction($documentId, $action, Request $request)
    {
        ...
        $upload_event = ProgramDetail::find($document->model_id);
        if (!$document) {
    ...
    public function performBulkAction(Request $request)
    {
        ...
        try {
            foreach ($documentIds as $id) {
                $document = ProgramDetail::find($id);
