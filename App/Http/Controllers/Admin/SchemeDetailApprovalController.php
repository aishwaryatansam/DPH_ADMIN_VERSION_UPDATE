...
class SchemeDetailApprovalController extends Controller
{
    public function index()
    {
        $userId = Auth::id();
        // dd($userId);
        $results = ApprovalWorkflows::getQueriedResult('App\Models\SchemeDetail');
        $pushedDocs = ApprovalWorkflows::getQueriedResult('App\Models\SchemeDetail', ['published']);
        $verifiedDocs = ApprovalWorkflows::getQueriedResult('App\Models\SchemeDetail', ['phc_verify', 'block_verify', 'hud_verify', 'state_verify']);
        $approvedDocs = ApprovalWorkflows::getQueriedResult('App\Models\SchemeDetail', ['phc_approve', 'block_approve', 'hud_approve', 'state_approve']);
        $pendingDocs = ApprovalWorkflows::getQueriedResult('App\Models\SchemeDetail', ['hsc_upload', 'phc_upload', 'block_upload', 'hud_upload', 'state_upload']);
        $returnedDocs = ApprovalWorkflows::getQueriedResult('App\Models\SchemeDetail', ['returned_with_remarks']);
        $rejectedDocs = ApprovalWorkflows::getQueriedResult('App\Models\SchemeDetail', ['rejected_with_remarks']);
        // dd($results['paginatedResult']->toarray());
        return view('admin.approval-management.schemedetail.list', compact('results', 'pushedDocs', 'verifiedDocs', 'approvedDocs', 'pendingDocs', 'returnedDocs', 'rejectedDocs', 'userId'));
    }
    ...
    public function handleAction($documentId, $action, Request $request)
    {
        ...
        $upload_event = SchemeDetail::find($document->model_id);
        if (!$document) {
