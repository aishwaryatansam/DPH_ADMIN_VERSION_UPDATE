...
class DocumentApprovalController extends Controller
{
    public function index()
    {
        $userId = Auth::id();
        // dd($userId);
        $results = ApprovalWorkflows::getQueriedResult('App\Models\NewDocument');
        $pushedDocs = ApprovalWorkflows::getQueriedResult('App\Models\NewDocument', ['published']);
        $verifiedDocs = ApprovalWorkflows::getQueriedResult('App\Models\NewDocument', ['phc_verify', 'block_verify', 'hud_verify', 'state_verify']);
        $approvedDocs = ApprovalWorkflows::getQueriedResult('App\Models\NewDocument', ['phc_approve', 'block_approve', 'hud_approve', 'state_approve']);
        $pendingDocs = ApprovalWorkflows::getQueriedResult('App\Models\NewDocument', ['hsc_upload', 'phc_upload', 'block_upload', 'hud_upload', 'state_upload']);
        $returnedDocs = ApprovalWorkflows::getQueriedResult('App\Models\NewDocument', ['returned_with_remarks']);
        $rejectedDocs = ApprovalWorkflows::getQueriedResult('App\Models\NewDocument', ['rejected_with_remarks']);
        return view('admin.approval-management.document.list', compact('results', 'pushedDocs', 'verifiedDocs', 'approvedDocs', 'pendingDocs', 'returnedDocs', 'rejectedDocs', 'userId'));
    }
    ...
    public function handleAction($documentId, $action, Request $request)
    {
        ...
        $upload_event = NewDocument::find($document->model_id);
        if (!$document) {
